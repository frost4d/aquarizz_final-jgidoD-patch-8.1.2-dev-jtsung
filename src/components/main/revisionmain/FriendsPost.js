  ...
  useEffect(() => {
    const fetchFollowedUsers = async () => {
      ...
      try {
        ...
        const followingCollection = collection(db, "users1", user.uid, "friends");
        const querySnapshot = await getDocs(followingCollection);

        // Get the followedUserId from the subcollection
        const followedUsersList = querySnapshot.docs.map((doc) => doc.data().friendId);
